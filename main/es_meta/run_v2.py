"""
ES Meta Optimizer V2 - with improved task embeddings and stochastic sampling.
"""
...
try:
    from .meta_learner_v2 import ESMetaLearnerV2
...
class ES_Meta_Optimizer_V2(BaseOptimizer):
    """
    ES Meta Optimizer V2 with improved embeddings and stochastic sampling.
    """

    def __init__(self, args=None):
        super().__init__(args)
        self.model_name = "es_meta_v2"
    ...
    def select_seeds(self, oracle_name: str, population_size: int) -> List[str]:
        ...
        print(f"Selected {len(seeds)} seeds using meta-learned scorer (T={self.temperature}, stochastic={self.stochastic})")
        return seeds
    ...
    def _save_experience(self, oracle_name: str, initial_seeds: List[str]):
        ...
        self.sort_buffer()
        top_items = list(self.mol_buffer.items())[:10]
        final_top10_avg = np.mean([item[1][0] for item in top_items])
        ...
        exp_path = os.path.join(self.experience_dir, f'{oracle_name.lower()}_exp_v2.pt')
        torch.save({
            'oracle_name': oracle_name,
            'seed_smiles': initial_seeds,
            'seed_embeddings': seed_embeddings,
...
if __name__ == '__main__':
    print("ES Meta Optimizer V2 loaded")
