"""
ES Meta Optimizer V4 - with adaptive temperature per task category.
"""
...
try:
    ...
    from .task_embeddings_v2 import get_task_embedding_v2, ORACLE_TO_CATEGORY
...
# Adaptive temperature per task category
# Lower temperature = more exploitation (use for well-trained categories)
# Higher temperature = more exploration (use for less-trained categories)
CATEGORY_TEMPERATURES = {
    'mpo': 0.08,        # Well-trained, exploit
    'bioactivity': 0.08,
    'property': 0.08,
    'similarity': 0.1,
    'rediscovery': 0.1,
    'hop': 0.15,        # Less trained, more exploration
    'isomers': 0.2,     # Very little training data
    'median': 0.2,
    'smarts': 0.15,
}
...
class ES_Meta_Optimizer_V4(BaseOptimizer):
    """
    ES Meta Optimizer V4 with adaptive temperature per task category.
    """
    ...
    def __init__(self, args=None):
        ...
        self.temperature = getattr(args, 'temperature', None)  # None = adaptive
        ...
        if self.meta_checkpoint and os.path.exists(self.meta_checkpoint):
            ...
            print(f"Loaded meta-learner V4 from {self.meta_checkpoint}")
    ...
    def get_adaptive_temperature(self, oracle_name: str) -> float:
        """Get adaptive temperature based on task category."""
        if self.temperature is not None:
            return self.temperature

        category = ORACLE_TO_CATEGORY.get(oracle_name, 'property')
        return CATEGORY_TEMPERATURES.get(category, 0.1)

    def select_seeds(self, oracle_name: str, population_size: int) -> List[str]:
        """Select seeds using meta-learned scorer with adaptive temperature."""
        ...
        temperature = self.get_adaptive_temperature(oracle_name)

        top_indices = self.meta_learner.select_seeds(
            all_embeddings, oracle_name, k=population_size,
            temperature=temperature, stochastic=self.stochastic
        )

        seeds = [self.all_smiles[i] for i in top_indices.tolist()]
        category = ORACLE_TO_CATEGORY.get(oracle_name, 'property')
        print(f"Selected {len(seeds)} seeds (category={category}, T={temperature:.2f})")
        return seeds
    ...
    def _save_experience(self, oracle_name: str, initial_seeds: List[str]):
        ...
        exp_path = os.path.join(self.experience_dir, f'{oracle_name.lower()}_exp_v4.pt')
        torch.save({
            'oracle_name': oracle_name,
            'seed_smiles': initial_seeds,
            'seed_embeddings': seed_embeddings,
...
if __name__ == '__main__':
    print("ES Meta Optimizer V4 loaded")
