"""
Train V3 meta-learner with contrastive objective and test on held-out oracles.
"""
...
def train_v3(experiences, n_epochs=300, output_path='main/es_meta/data/meta_scorer_v4.pt'):
    """Train V3 meta-learner."""
    print(f"\nTraining V3 meta-learner on {len(experiences)} experiences...")

    learner = ESMetaLearnerV3(population_size=30, noise_std=0.03, lr=0.15)
...
class Args:
    ...
    smi_file = None
    n_jobs = -1
    output_dir = 'main/es_meta/data/test_results_v3'
    patience = 5
...
if '{method}' == 'graph_ga':
    ...
    from main.es_meta.run_v3 import ES_Meta_Optimizer_V3
    optimizer = ES_Meta_Optimizer_V3(args=Args())
...
def run_comparison(oracles, seeds, max_calls, meta_checkpoint):
    """Compare graph_ga vs es_meta_v3."""
    results = {'graph_ga': {}, 'es_meta_v3': {}}
    ...
    for oracle in oracles:
        ...
        results['es_meta_v3'][oracle] = []

        for seed in seeds:
            for method in ['graph_ga', 'es_meta_v3']:
                count += 1
                ...
                if method == 'graph_ga':
                    ...
                    res = test_single('es_meta_v3', oracle, seed, max_calls, meta_checkpoint)
...
def print_summary(results, oracles):
    ...
    print("="*70)
    print(f"{'Oracle':<30} {'graph_ga':>10} {'es_meta_v3':>12} {'Diff':>10}")
    print("-"*70)
    ...
    for oracle in oracles:
        ...
        es = [r['top10_avg'] for r in results['es_meta_v3'].get(oracle, []) if 'top10_avg' in r]
    ...
    print(f"\nes_meta_v3 wins: {wins}/{len(oracles)}")
...
def main():
    ...
    parser.add_argument('--output', default='main/es_meta/data/meta_scorer_v4.pt')
    ...
    if not args.skip_train:
        learner = train_v3(experiences, n_epochs=args.epochs, output_path=args.output)
    else:
    ...
    # Save results
    with open('main/es_meta/data/comparison_v3.json', 'w') as f:
        json.dump(results, f, indent=2)

    print(f"\nResults saved to main/es_meta/data/comparison_v3.json")
